...
{
//TODO Base case
if (node == NULL)   //  ends the loop of the recursion
    return 0;
//TODO Implement Sum of all nodes
    int sum = node -> data;     // sets value to current node
    sum += sumNodes(node -> left);  // adds the values of the children on left
    sum += sumNodes(node -> right); //adds the values of children on right
