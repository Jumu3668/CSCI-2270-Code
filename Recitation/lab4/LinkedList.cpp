...
bool LinkedList::deleteAtIndex(int n)
{
  ...
  // Special case to delete the head
    if (n == 0) {
        //TODO
        //set header to second node. header already points to first node, so we say header -> next for the second node.
        // temp stores pointer to first node so we can delete it.
        Node *temp = head;
        head = head -> next;
        delete temp;
        isDeleted = true;
