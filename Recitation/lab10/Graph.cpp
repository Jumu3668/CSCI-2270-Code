...
void Graph::removeEdge(int key1, int key2)
{
  ...
  // Ex: say you need to remove i'th index item from a vector "myvector", you can use myvector.erase(myvector.begin()+i)
  // v2 = key2
  // v1 = key1
  //CASE remove pointer to v2 in v1 adjacency list
  for (int i = 0; i < v1 -> adj.size(); i++)
  {
      if (v1 -> adj[i].v -> key == v2 -> key)
      {
          //remove pointer to v2
          v1 -> adj.erase(v1 -> adj.begin() + i);
      }
  }
  //CASE remove pointer to v1 in v2 adjacency list
  for (int i = 0; i < v2 -> adj.size(); i++)
  {
      if (v2 -> adj[i].v -> key == v1 -> key)
      {
          //remove pointer to v1
